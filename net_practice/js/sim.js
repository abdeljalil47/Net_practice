...
function ip_match_route(ip, r) {
    ...
    if (r['h']['type'] == "internet" && str == '0.0.0.0/0') { g_sim_logs += 'invalid default route on internet ' + r['hid'] + '\n'; return (0); }
    var tab = str.split('/');
    //    my_console_log("ip_match_route check : "+str+" againt ip "+ip);
    if (tab.length != 2) { g_sim_logs += 'invalid route on host ' + r['hid'] + '\n'; return (0); }
    if ((rip = ip_to_int(tab[0])) === null) { g_sim_logs += 'invalid route on host ' + r['hid'] + '\n'; return (0); }
    if ((rmask = mask_to_int('/' + tab[1])) === null) { g_sim_logs += 'invalid route on host ' + r['hid'] + '\n'; return (0); }
...
function rec_route(ip_dest, local_target, input_itf, h)   // return array of dest itf
{
    ...
    if (h['type'] == 'switch') {
        ...
        ret = [];
        links.forEach(l => {
            if (l['e1']['hid'] == h['id']) ret = ret.concat(rec_route(ip_dest, local_target, l['e2'], l['h2']));
    ...
    if (h['type'] == 'internet') {
        if (ip_match_route(ip_dest, private_subnets[0]) || ip_match_route(ip_dest, private_subnets[1]) || ip_match_route(ip_dest, private_subnets[2])) { g_sim_logs += 'private subnets not routed over internet\n'; return ([]); }
    }
    ...
    for (i = 0; i < ifs.length; i++) {
        if (ifs[i]['hid'] == h['id']) {
            ...
            if (ip_match_if(ip_dest, ifs[i])) {
                ...
                links.forEach(l => {
                    if (l['if1'] == ifs[i]['if']) ret = ret.concat(rec_route(ip_dest, ip_dest, l['e2'], l['h2']));
                ...
                for (i = 0; i < ifs.length; i++) {
                    if (ifs[i]['hid'] == h['id']) {
                        if (ip_match_if(ip_gate, ifs[i])) {
                            ...
                            nbif++;
                            links.forEach(l => {
                                if (l['if1'] == ifs[i]['if']) ret = ret.concat(rec_route(ip_dest, ip_gate, l['e2'], l['h2']));
